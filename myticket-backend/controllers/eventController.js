...
export const getEventImages = async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      "SELECT imagePath FROM event_images WHERE idEvenement = ?",
      [eventId]
    );
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "Aucune image trouvée pour cet événement." });
    }
...
export const getEventById = async (req, res) => {
  ...
  try {
    const [event] = await db.query(
      "SELECT idEvenement, nom, date, lieu, description, candyMachineId FROM evenement WHERE idEvenement = ?",
      [id]
    );
